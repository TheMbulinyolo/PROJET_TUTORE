...
Private Sub LancerPythonSansMessage()
    ...
    exePath = CheminRacineProjet() & "\" & EXECUTABLE_MOTEUR
    sortie = CheminSortie()

    If Not FichierExiste(exePath) Then
        Err.Raise vbObjectError + 1001, "LancerCalculPython", _
            "Moteur de calcul introuvable : " & exePath
    End If
    ...
    commande = Q(exePath) & " " & Q(CheminEntree()) & " " & Q(sortie)
    ...
    If Not FichierExiste(sortie) Then
        Err.Raise vbObjectError + 1003, "LancerCalculPython", _
            "Le moteur de calcul n'a pas crée le fichier de sortie " & vbCrLf & _
            "Code retour : " & codeRetour & vbCrLf & _
            "Commande : " & commande
    End If
    If codeRetour <> 0 Then
        Err.Raise vbObjectError + 1004, "LancerCalculPython", _
            "Le moteur de calcul signale une erreur (code " & codeRetour & ")." & _
